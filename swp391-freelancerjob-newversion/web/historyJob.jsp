<%-- 
    Document   : historyJOb
    Created on : Jul 8, 2022, 5:04:15 PM
    Author     : Zenos
--%>

<%@page import="sample.feedback.FeedbackDTO"%>
<%@page import="sample.feedback.FeedbackDAO"%>
...
<%@page import="sample.history.HistoryDAO"%>
            ...
            <div class="container mt-5">
                <div class="list">
                    <% HistoryDAO historyDao = new HistoryDAO();
                        ...
                        List<HistoryDTO> listHistory = historyDao.getHistoryListByAccountID(loginUser.getAccountID());
                    %>

                    <%
                        for (HistoryDTO historyList : listHistory) {
                            JobDTO job = user.getJobByJobID(historyList.getJobID());
                            String[] parts1 = job.getStartTime().split("-");
                            String start_date = parts1[2] + "-" + parts1[1] + "-" + parts1[0];
                            String[] parts = historyList.getDate_end().split("-");
                            String final_date = parts[2] + "-" + parts[1] + "-" + parts[0];
                            String[] time = historyList.getTime_end().split("\\.");
                            String final_time = time[0];
                            String price = jobDao.formatPrice(job.getPrice());
                            FeedbackDTO feedback1 = feedback.getFeedbackByJobID(historyList.getJobID());
                    %>
                    <%
                        if (historyList.getAccountID_employer().equals(loginUser.getAccountID())) {
                    %>
                    <div class="post">
                        <p id="title">Tên dự án: <%= job.getJobName()%></p>
                        <p id="owner">Tên nhà tuyển dụng:  </p>
                        <p id="detail"><%= job.getDescription()%></p>
                        <p id="price"> <span> <%= price%> VNĐ</span></p>
                        <p id="time">Từ <%= start_date%> đến <%= final_date%></p>
                        <%
                            
                            if (feedback1 != null) {
                        %>
                        <p id="feedback">Đánh giá từ <%= historyList.getAccountID_employer() %>: <%= feedback1.getFeedback() %></p>
                        <%
                        }
                        %>
                    </div>    
                        ...
                        %>
                        <%
                            if (historyList.getAccountID_employee().equals(loginUser.getAccountID())) {
                        %>
                        <div class="post">
                        <p id="title">Tên dự án: <%= job.getJobName()%></p>
                        <p id="owner">Tên nhà tuyển dụng: <%= historyList.getAccountID_employer()%>  </p>
                        <p id="detail"><%= job.getDescription()%></p>
                        <p id="price"> <span> <%= price%> VNĐ</span></p>
                        <p id="time">Từ <%= start_date%> đến <%= final_date%></p>
